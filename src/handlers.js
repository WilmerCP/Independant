...
handler._users.POST = function (data, callback) {
  ...
  if (firstName && lastName && hashedPassword && phoneNumber && tosAgreement) {
    ...
    _data.create("users", phoneNumber, user).then((e) => {
      if (e) {
        callback(500, {
          Error: "Could not store the data, the user may already exist",
        });
      } else {
...
handler._users.PUT = function (data, callback) {
  ...
  if (phoneNumber) {
    if (firstName || lastName || password) {
      _data.read("users", phoneNumber).then((res) => {
        if (res) {
          if (firstName) {
          ...
          _data.update("users", phoneNumber, res).then((e) => {
            if (e) {
              callback(500, {
                Error: "Could not store the data, the user may already exist",
              });
...
handler._users.DELETE = function (data, callback) {
  ...
  if (phoneNumber) {
    _data.delete("users", phoneNumber).then((res) => {
      if (!res) {
...
handler._users.GET = function (data, callback) {
  ...
  if (phoneNumber) {
    let user = _data.read("users", phoneNumber).then((res) => {
      if (res) {
...
handler._tokens.POST = function (data, callback) {
  ...
  let password = helpers.validPassword(payload.password);
  let hashedPassword =
    typeof password === "string" ? helpers.hash(password) : false;
...
handler._tokens.PUT = function (data, callback) {
  ...
  let extend =
    typeof data.content.extend === "boolean" ? data.content.extend : false;
...
handler._tokens.DELETE = function (data, callback) {
  ...
  if (tokenId) {
    _data.delete("tokens", tokenId).then((res) => {
      if (!res) {
        callback(200, { Nice: "The DELETE operation was succesful" });
      } else {
        callback(500, {
          Error: "Could not delete the token, it may not exist",
        });
      }
    });
  } else {
